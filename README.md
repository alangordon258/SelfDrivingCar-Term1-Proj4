...
The next step was to create a thresholded, binary image. I started out following the steps shown in class. I applied a combination of Sobel gradients and color gradients and spent a great deal of time tuning the gradients and color thresholds. I ultimately came to the conclusion that using only color thresholds and eschewing the sobel gradients was better. The conversation in the class Slack channel suggested that many others in the class reached the same conclusion. 
I then used the logic I had written to warp and generate a thresholded binary image for each image in this set. The image below shows my full set of test images run against my pipeline using the best parameters I could come up with for each of the Sobel gradients plus a color threshold. The color threshold in this case was the L channel from a LUV colorspace transformation with a threshold of 220<L<=255 and the B channel from an LAB colorspace transformation with a threshold of 155<B<=200. 
![alt text][image5]
I also tried using only the color thresholds, which yielded the following result.
![alt text][image4]
Using the gradients only yielded the following.
![alt text][image6]
I found the best solution was to use the L and B channels as described above, but to use a "smart" threshold on the B channel. The B channel represents a range of colors from blue to yellow.  To isolate yellow, wscale if the max value on the b_channel is greater than 175. This yielded the result shown below.
![alt text][image7]
The result using this approach was not perfect. Notice that the left (yellow) line is not visible in the last 3 images. Keep in mind that these last 3 images were chosen precisely because they were problematic, so I didn't expect them to work all the time. Moreover, my line tracking algorithm is tolerant of one or both lines being missed for some frames.
...
I added diagnostics and observed the failure mode when I got a bad curve fit and most of the times a symptom of a bad fit is that the left and right curvatures were wildly different from each otehr.
